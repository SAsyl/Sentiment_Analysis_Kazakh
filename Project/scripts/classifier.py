...
def tfidf_distribution(df, test_size, plot):
    ...
    if plot:
        ...
        plt.legend()
        plt.savefig(f"./../results/classifier/tfidf_distibution_test-size({test_size}).png")

    return train, test


def plot_ROC_AUC(test, predict_lr_proba):
...
def plot_confusion_matrix(dataset, predict_proba, threshold):
    ...
    plt.savefig(f"./../results/classifier/confusion_matrix_{threshold:.3f}.png")
...
def main():
    df = pd.read_csv(normalized_comments_path)
    print(df['label'].head())

    # classifier = Classifier(df)

    train, test = tfidf_distribution(df=df, test_size=0.2, plot=False)
